...
As we can see the Cross Validation MSE increases exponentially for each increase in lambda. As said in the assignment the tuningparameter that we call lambda pushes the beta ridge estimates to 0 as it increases. Here it seems that the beta ridge estimates seems to fit the model well, as the increase in lambda leads to a higher MSE. Therefore, from the results shown in the plot, it seems like we should use a small lambda as possible, preferably 0 when we try to fit a model into the used sample.  
\end{document}